...
export const createField = (fields = {}) => {
  ...
  const helpText = (name, touched, errors) => (touched[name] && errors[name]) && <HelpText variant="danger">{errors[name]}</HelpText>;

  const CustomFormGroup = ({ label, touched, errors, name, showLabel, children }) => (
    <FormGroup>
  ...
  const CustomInput = ({ 
    ...
    <CustomFormGroup {...form} label={label} showLabel={showLabel} name={field.name}>
      <Input 
        {...field} 
        {...props} 
        label={label}
  ...
  const CustomSelect = ({
    ...
    <CustomFormGroup {...formOthers} label={label} showLabel={showLabel} name={field.name}>
      <Select
        {...field}
        {...props}
